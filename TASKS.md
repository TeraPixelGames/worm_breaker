...
- [x] [P2] Add at least one meaningful run-time choice (risk/reward path, loadout, or powerup branch).
- [x] [P2] Increase variety with rotating pattern/enemy/modifier pools while preserving core rules.
- [x] [P1] Define pacing phases (warm-up, pressure peak, cooldown) with explicit timer/state transitions.
- [x] [P2] Add social/emotional hook: leaderboard season, async challenge card, or shareable rival target.
- [x] Completed wave 2 gameplay systems pass (`scripts/run-tests.ps1 -Suite unit`).
- [x] Completed wave 3 cohesion + rival social hook pass (scripts/run-tests.ps1 -Suite unit).
...
- [x] Fixed strict Variant inference warnings in `GameController.gd` exposed by visual smoke.
