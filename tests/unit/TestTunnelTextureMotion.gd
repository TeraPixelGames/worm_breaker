...
func test_audio_bpm_onset_requires_sharp_pulse_rise() -> void:
	...
	assert_true(not GameController.audio_beat_onset(0.3, 0.26), "Sustained pulse levels should not retrigger BPM")
	assert_true(not GameController.audio_beat_onset(0.12, 0.0), "Small pulse rises should stay below the BPM gate")
...
func test_audio_bpm_confidence_attacks_and_decays() -> void:
	...
	assert_true(attacked > 0.0, "Valid beat intervals should increase BPM confidence")
...
func test_audio_bpm_can_raise_but_not_lower_tunnel_speed() -> void:
	...
	var synced_speed: float = GameController.tunnel_texture_speed_for_audio_bpm(base_speed, 170.0, 1.0, 0.5)
	var quiet_speed: float = GameController.tunnel_texture_speed_for_audio_bpm(base_speed, 90.0, 1.0, 0.0)
	assert_true(synced_speed > base_speed, "High-confidence fast BPM should raise tunnel texture speed")
